...
def get_classes_percentage_stats():
    ...
    """
    Devuelve los datos de porcentaje de asistencia por clase y por mes.
    """
    # Obtener parámetros
    current_year  = datetime.now().year
    selected_year = request.args.get('year', type=int, default=current_year)
    ...
    # Determinar qué clases incluir
    classes_to_compute = set()
    if class_code1 == "all":
        # Si se selecciona "all", obtener todas las clases disponibles.
        query = db.session.query(Attendance.class_code).distinct().filter(Attendance.meeting_center_id == meeting_center_id)
    ...
    for code in classes_to_compute:
        # Obtener registros de asistencia para la clase y el año seleccionado
        records = db.session.query(Attendance).filter(
            Attendance.class_code == code, Attendance.meeting_center_id == meeting_center_id,
            func.extract("year", Attendance.sunday_date) == selected_year
        ).order_by(Attendance.class_code).all()
        
        # Agrupar por sesión (fecha) usando un diccionario
        sessions = defaultdict(set)
        for rec in records:
            # Usamos la fecha completa (o solo la fecha sin la hora)
            session_date = rec.sunday_date
            sessions[session_date].add(rec.student_name)
        
        # Agrupar las sesiones por mes
        ...
        for m in range(1, 13):
            if m in sessions_by_month and sessions_by_month[m]:
                # Ordenar sesiones del mes por fecha y tomar la primera como baseline
                sorted_sessions = sorted(sessions_by_month[m], key=lambda x: x[0])
                baseline = sorted_sessions[0][1]
                baseline_count = len(baseline)
                if baseline_count == 0:
                    # Si no hay baseline válido, se omite este mes
                ...
                # Excluir sesiones sin datos (si se desea, se puede filtrar a nivel de gráfico)
...
def stats():
    ...
    query = db.session.query(
        Attendance.sunday_date, Classes.class_code, Classes.class_color, db.func.count(Attendance.id)
    ...
    for date, class_code, color, count in data:
    ...
    for class_code, date_counts in chart_data.items():
        data_points = []

        for label in labels:
             data_points.append(date_counts.get(label))

        datasets.append({
            "label": _(class_code),  # Ahora usa el código en lugar del nombre
            "data": data_points,
            "fill": False,
            "borderWidth": 1,
            "spanGaps": True
